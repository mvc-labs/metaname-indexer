...
const OP_PUSH_LEN = 2;
const GENESISTX_ID_OFFSET = ProtoHeader.PROTO_HEADER_OFFSET + ProtoHeader.GENESIS_TXID_LEN
const GENESIS_HASH_OFFSET = GENESISTX_ID_OFFSET + ProtoHeader.HASH_LEN
...
export function getGenesisID(script: Buffer) {
...
export function getNftID(script: Buffer) {
    return mvc.crypto.Hash.sha256ripemd160(script.subarray(script.length - TOKEN_INDEX_OFFSET, script.length - GENESISTX_ID_OFFSET + ProtoHeader.GENESIS_TXID_LEN))
}
...
export function getMetaidOutpoint(script: Buffer) {
    return script.subarray(script.length - METAID_OUTPOINT_OFFSET, script.length - METAID_OUTPOINT_OFFSET + METAID_OUTPOINT_LEN)
}
...
export function getScriptRawData(script: Buffer) {
    return script.subarray(script.length - RAW_DATA_LEN, script.length)
}
...
export function getNewGenesisScript(script: Buffer, sensibleID: Buffer, tokenIndex: bigint) {
    ...
    return Buffer.concat([
        ...
        indexBuf,
        script.subarray(script.length - GENESIS_HASH_OFFSET, script.length - GENESISTX_ID_OFFSET),
        sensibleID,
        script.subarray(script.length - GENESISTX_ID_OFFSET + ProtoHeader.GENESIS_TXID_LEN, script.length)
    ])
}

export function getGenesisRaw(script: Buffer) {
    return script.subarray(script.length - GENESIS_HASH_OFFSET, script.length - GENESISTX_ID_OFFSET + ProtoHeader.GENESIS_TXID_LEN)
}